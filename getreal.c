...
void cipher(char* pattern, uint16_t psize) {
...
int main(int argc, char** argv) {
    ...
    fread(buffer, remaining, 1, infile);
    fwrite(buffer, remaining, 1, outfile);
    ...
    for (pidx = 0; pidx < patnum; pidx++) {
        ...
        char pattern [psize];
        fread(pattern, psize, 1, outfile);
        fseek(outfile, -psize, SEEK_CUR);
        cipher(pattern, psize);                 // de(/en)crypt the data
        fwrite(pattern, psize, 1, outfile);
